...
class TrelloData(models.Model):
    ...
    def get_trello_projects(self, user=None):
        ...
        if user or self.env.user:
            trello_username = (
                user.trello_username
                if user and user.trello_username
                else self.env.user.trello_username
            )
            trello_api_key = (
                user.trello_api_key
                if user and user.trello_api_key
                else self.env.user.trello_api_key
            )
            trello_api_token = (
                user.trello_api_token
                if user and user.trello_api_token
                else self.env.user.trello_api_token
            )
    ...
    def get_trello_cards(self, user=None):
        ...
        if user or self.env.user:
            ...
            if trello_api_key and trello_api_token:
                for project_id in project_data:
                    task_url = (
                        "https://api.trello.com/1/boards/" + project_id + "/cards"
                    )
                    task_querystring = {
                        "key": trello_api_key,
                        "token": trello_api_token,
                    ...
                    if task_response:
                        ...
                        for data in task_datas:
                            ...
                            attachment_response = requests.request(
                                "GET", attachment_url, params=attachment_querystring
                            )
                                        ...
                                        self.env["ir.attachment"].create(
                                            {
                                                ...
                                                "res_id": res_id,
                                                "trello_attachment_id": attachment[
                                                    "id"
                                                ],
                                            }
                                        )
                ...
                for project in projects:
                    if project.trello_project_id:
                        ...
                        project_response = requests.request(
                            "PUT", project_url, params=project_querystring
                        )
                        if project_response:
                        ...
                        for stages in project.type_ids:
                            if stages.trello_stage_id:
                                ...
                                stage_response = requests.request(
                                    "PUT", stage_url, params=stage_querystring
                                )
                            else:
                                ...
                                stage_response = requests.request(
                                    "POST", stage_url, params=stage_querystring
                                )
                            if stage_response:
                        ...
                        for task in project.task_ids:
                            ...
                            for tag in task.tag_ids:
                                ...
                                if tag.trello_tag_id:
                                    ...
                                    tag_response = requests.request(
                                        "PUT", tag_url, params=tag_querystring
                                    )
                                else:
                                    ...
                                    tag_response = requests.request(
                                        "POST", tag_url, params=tag_querystring
                                    )
                                if tag_response:
                            ...
                            if task.trello_task_id:
                                ...
                                task_querystring = {
                                    "idList": task.stage_id.trello_stage_id,
                                    "name": task.name,
                                    "pos": task.sequence,
                                    "desc": TAG_RE.sub("", task.description)
                                    if task.description
                                    else "",
                                ...
                                task_response = requests.request(
                                    "PUT", task_url, params=task_querystring
                                )
                            else:
                                ...
                                task_response = requests.request(
                                    "POST", task_url, params=task_querystring
                                )
                            if task_response:
                            ...
                            for attachment in task.attachment_ids:
                                if not attachment.trello_attachment_id:
                                    file_path = attachment._full_path(
                                    ...
                                    if attachment_response:
                                        attachment_datas = json.loads(
                                            attachment_response.text
                                        )
                                        attachment.trello_attachment_id = (
                                            attachment_datas["id"]
                                        )
                        ...
                        project_response = requests.request(
                            "POST", project_url, params=project_querystring
                        )
                        print(
                            "project_responsejjjjjjjjjjjjjj",
                            project_response,
                            project.name,
                        )
                        if project_response:
            ...
            if trello_api_key and trello_api_token:
                ...
                for project_id in project_data:
                    ...
                    if task_response:
                        task_datas = json.loads(task_response.text)
                    stage_url = (
                        "https://api.trello.com/1/boards/" + project_id + "/lists"
                    )
                    stage_querystring = {
                        "key": trello_api_key,
                        "token": trello_api_token,
                    ...
                    stage_response = requests.request(
                        "GET", stage_url, params=stage_querystring
                    )
                    if stage_response:
                ...
                for stage in stages:
                    if stage.trello_stage_id not in trello_stage_ids:
                        existing_stage = self.env["project.task.type"].search(
                            [
                                ("trello_stage_id", "=", stage.trello_stage_id),
                                ("trello_stage_id", "!=", False),
                            ]
                        )
                        if existing_stage:
