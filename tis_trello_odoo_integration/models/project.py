...
class Project(models.Model):
    ...
    def unlink(self):
        if self.trello_project_id:
            ...
            querystring = {
                "key": self.env.user.trello_api_key,
                "token": self.env.user.trello_api_token,
            }
        ...
        for stages in self.type_ids:
            if stages.trello_stage_id:
                ...
                stage_querystring = {
                    "name": stages.name,
                    "pos": "bottom",
                    "idBoard": self.trello_project_id,
                    "key": api_key,
                    "token": api_token,
                }
...
class ProjectTags(models.Model):
    ...
    project_id = fields.Many2one(
        "project.project", string="Project", required=True, copy=False
    )
    _sql_constraints = [
