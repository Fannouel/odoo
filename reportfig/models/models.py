...
class reportfig(models.Model):
     ...
     origin = fields.Char( string="N°Bon Command", related='NFabric_id.BC.name', readonly= True)
     ...
     Urgence = fields.Selection(string="Urgence",related='NFabric_id.stateFabric', readonly= True)
     QttCommander = fields.Float(string="Quantite Command",related='NFabric_id.product_qty')
     Etat = fields.Selection([('L', 'Lancé en Fabrication'), ('A', 'en attente'), ('T', 'Términé')], string='Etat Fabric', store=True)
     Quotat = fields.Integer(string="Quotat")
     NbrDeFiche = fields.Integer(string="Nbr de Fiche", compute="_nbrfiche", store="True")
     ResteFiche = fields.Integer(string="Reste Article",compute="_restfiche", store="True")
