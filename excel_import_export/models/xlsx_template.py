...
class XLSXTemplate(models.Model):
    ...
    pre_import_hook = fields.Char(
        string="Pre Import Function Hook",
        help="Call a function before successful import, i.e.,\n"
        "${object.pre_import_do_something()}",
    )
    ...
    def create(self, vals_list):
        ...
        for rec in res:
            if rec.input_instruction:
                ...
                rec._compute_input_pre_import_hook()
...
self['{}'] = self.env['{}'].search(self.safe_domain(self.domain))
    ...
    def _compute_input_pre_import_hook(self):
        ...
        for rec in self:
            # Import Instruction
            input_dict = literal_eval(rec.input_instruction.strip())
            rec.pre_import_hook = input_dict.get("__PRE_IMPORT__")
    ...
    def _compute_output_instruction(self):
        ...
        for rec in self:
            ...
            itype = "__PRE_IMPORT__"
            inst_dict[itype] = False
            if rec.pre_import_hook:
                inst_dict[itype] = rec.pre_import_hook
